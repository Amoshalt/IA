...
class Neuron:
  ...
  def learn(self,eta,sigma,posxbmu,posybmu,x):
    ...
    '''
    # TODO
    j = numpy.array([self.posx, self.posy])
    j_star = numpy.array([posxbmu, posybmu])
    gaussian = numpy.exp(-((numpy.linalg.norm(j-j_star))**2)/(2*(sigma**2)))
    self.weights += eta*gaussian*(x - self.weights)
